...
pub struct SettleQuest<'info> {
    ...
    /// CHECK: Must be quest.publisher and receives expired-task refunds.
    #[account(mut)]
    pub publisher: UncheckedAccount<'info>,

    /// CHECK: Anyone may settle, but the fee vault must already exist from create/fund/close.
    #[account(mut, seeds = [FEE_VAULT_SEED], bump)]
    pub fee_vault: UncheckedAccount<'info>,

    /// CHECK: Anyone may settle, but the public goods pool must already exist from create/close.
...
pub fn settle_quest_handler(ctx: Context<SettleQuest>) -> Result<()> {
    ...
    require!(
        ...
        ErrorCode::PendingSubmissionsRemaining
    );
    require!(
        quest.next_review_index == quest.next_submission_index,
        ErrorCode::UnreviewedSubmissionsRemaining
    );
    require!(
